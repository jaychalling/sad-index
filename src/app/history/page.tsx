...
import { bsiWeeklyData, historicalEvents } from '@/data/bsi-data'
                ...
                return (
                  <div key={event.date} className="flex gap-4 md:gap-8 items-start">
                    ...
                    <div className="hidden md:flex flex-col items-center flex-shrink-0 w-16">
                      <div
                        className="w-5 h-5 rounded-full border-[3px] border-navy z-10"
                        style={{ backgroundColor: bsiColor }}
                      />
                    </div>

                    {/* Card */}
                    <div
                      className="flex-1 bg-white rounded-lg p-5 md:p-6 border-[3px] border-navy"
                      style={{ boxShadow: i % 2 === 0 ? '4px 4px 0 #023047' : '4px 4px 0 #219ebc' }}
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                        <div>
                          <span className="tag-brutal bg-ocean/30 text-navy text-xs mr-2">
                            {month} {year}
                          </span>
                          <h3 className="inline text-lg font-bold text-navy" style={{ fontFamily: 'var(--font-poppins)' }}>
                            {event.label}
                          </h3>
                        </div>
                        <div
                          className="tag-brutal font-bold text-sm"
                          style={{ backgroundColor: bsiColor + '30', color: '#023047' }}
                        >
                          BSI {event.bsi}
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <div className="flex-1 h-3 bg-navy/10 rounded-full overflow-hidden border border-navy/20">
                          <div
                            ...
                            style={{ width: `${event.bsi}%`, backgroundColor: bsiColor }}
                          />
                        </div>
                        <span className="text-xs font-semibold text-navy/60 w-10 text-right">
                          {event.bsi}/100
                        </span>
                      </div>
                    </div>
